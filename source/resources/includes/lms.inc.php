...
function &lmsPageNewGrab() {
	$returnArray = array(
		'title'           => 'Damn Vulnerable Web Application (DVWA) v' . lmsVersionGet() . '',
		'title_separator' => ' :: ',
		'body'            => '',
		'page_id'         => '',
		'help_button'     => '',
...
function lmsHtmlEcho( $pPage ) {
	...
	if( lmsIsLoggedIn() ) {
		...
		$menuBlocks[ 'home' ][] = array( 'id' => 'setup', 'name' => 'Setup lms', 'url' => 'setup.php' );
		$menuBlocks[ 'home' ][] = array( 'id' => 'instructions', 'name' => 'Hướng dẫn', 'url' => 'instructions.php' );
	}
	...
	if( lmsIsLoggedIn() ) {
		$menuBlocks[ 'meta' ][] = array( 'id' => 'security', 'name' => 'lms Security', 'url' => 'security.php' );
	...
	$phpIdsHtml   = '<em>PHPIDS:</em> ' . ( lmsPhpIdsIsEnabled() ? 'enabled' : 'disabled' );
	$userInfoHtml = '<em>Người dùng:</em> ' . ( lmsCurrentUser() );
	...
	if( $pPage[ 'source_button' ] ) {
		$systemInfoHtml = lmsButtonSourceHtmlGet( $pPage[ 'source_button' ] ) . " $systemInfoHtml";
	}
	if( $pPage[ 'help_button' ] ) {
		$systemInfoHtml = lmsButtonHelpHtmlGet( $pPage[ 'help_button' ] ) . " $systemInfoHtml";
	}
